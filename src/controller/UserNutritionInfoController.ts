...
class UserNutritionInfoController {
  ...
  getByDate = asyncHandler(async (req: GlobalRequest, res: Response, next: NextFunction) => {
    let date = new Date(req.body.date);
    const userInfo = userNutritionInfoService.getByDate(req.user, date);
    res.status(200).json(userInfo);
  });
  ...
  patchUpdate = asyncHandler(async (req: GlobalRequest, res: Response, next: NextFunction) => {
    console.log(req)
